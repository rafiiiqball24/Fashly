        ...
        <div className="space-y-4">
          <div className="relative aspect-square overflow-hidden rounded-lg border">
            <Image
              ...
              alt={product.name}
              fill
              className={`object-cover transition-all duration-300 ${imageTransition ? "opacity-70 scale-105" : "opacity-100 scale-100"}`}
              priority
            />
          ...
          <div className="flex space-x-2">
            {product.images.map((image, index) => (
              <button
                key={index}
                className={`relative h-20 w-20 overflow-hidden rounded-md border transition-all duration-200 ${currentImage === image ? "ring-2 ring-black scale-105" : "hover:scale-105"}`}
            ...
            <RadioGroup value={selectedColor} onValueChange={handleColorChange} className="flex flex-wrap gap-3">
              {product.colors.map((color) => (
                <div key={color} className="flex items-center space-x-2">
                  ...
                  <Label
                    htmlFor={`color-${color}`}
                    className={`h-10 w-10 rounded-full border-2 cursor-pointer flex items-center justify-center transition-all duration-200 hover:scale-110 ${selectedColor === color ? "ring-2 ring-black ring-offset-2 scale-110" : "hover:ring-1 hover:ring-gray-300"}`}
            ...
            <RadioGroup value={selectedSize} onValueChange={setSelectedSize} className="flex flex-wrap gap-2">
              {product.sizes.map((size) => (
                <div key={size} className="flex items-center space-x-2">
                  ...
                  <Label
                    htmlFor={`size-${size}`}
                    className={`h-10 w-10 rounded-md border cursor-pointer flex items-center justify-center transition-all duration-200 hover:scale-105 ${selectedSize === size ? "bg-black text-white" : "bg-white text-black hover:bg-gray-50"}`}
                  >
                    {size}
                  </Label>
                </div>
